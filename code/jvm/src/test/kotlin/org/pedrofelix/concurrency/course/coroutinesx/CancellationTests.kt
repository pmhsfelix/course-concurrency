...
import org.pedrofelix.concurrency.course.utils.JobState
import org.pedrofelix.concurrency.course.utils.getState
...
class CancellationTests {
    ...
    /*
     * Takeaways:
     * - The inner coroutine is considered canceled even if it doesn't throw any exception and does
     *   return a value (the 42).
     * - More, it is considered canceled immediately after the `cancel`, even before it returns.
     ...
     *   - The first coroutine was not canceled because it completed before the `cancel`
     *   - The second coroutine is canceled, however it's still not completed when the cancel returns,
    ...
    /*
     * Takeaways:
     * - Using supervisorScope prevents the parent from being canceled even if one of the children is canceled
     */
    ...
    @Test
    fun `cancelling root scope does throw on runBlocking`() {
    ...
    @Test
    fun `cancelling child coroutine does not throw on runBlocking`() {
        runBlocking {
            logger.info("starting")
            val job = launch {
                delay(1000)
    ...
    @Test
    fun `cancelling child scope does throw on runBlocking`() {
        assertThrows<CancellationException> {
            runBlocking {
                logger.info("starting")
                coroutineScope {
